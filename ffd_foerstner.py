...
def detect_foerstner_kp(gray):
    w, q = corner_foerstner(gray, sigma=1)
    accuracy_thresh = 0.1
    roundness_thresh = 0.3
    foerstner = (q > roundness_thresh) * (w > accuracy_thresh) * w
    peaks = corner_peaks(foerstner, min_distance=5)

    #преобразование координат в KeyPoint
    kp_foerstner = []
    for el in peaks:
        kp_foerstner.append(cv2.KeyPoint(float(el[1]), float(el[0]), size=6))

    #img_foerstner=cv2.drawKeypoints(gray,kp_foerstner,img)
    #cv2.imwrite('foerstner_keypoints.jpg',img_foerstner)
    return kp_foerstner

#input_img = cv2.imread('HSeq_londonbridge_6.ppm', 0).astype(np.float32)/255.
def exp_curve(x, a, b):
    return a*np.exp(b*x)

def compute_FFD_keypoints(img_name):
    #input_img = cv2.imread(img_name, 0).astype(np.float32)/255.
    input_img = img_name.astype(np.float32)/255.
